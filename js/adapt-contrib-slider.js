define(function(require) {
    ...
    var Slider = QuestionView.extend({
        events: {
            ...
            "click .slider-widget .button.model": "tutor",
        ...
        onDragReleased: function (event) {
            ...
            $(this.$(".slider-items")).hide();
            $(document).off('mousemove touchmove');
            var itemIndex = this.getIndexFromValue(this.getSelectedItems().value);
            console.log(this.$el.find(".slider-items")[0]);
            var length=this.model.get("items").length
            for(var i=0;i<length;i++)
            {
                if(i==itemIndex){
                    var sliderItems=this.$(".slider-items")[i];
                    $(sliderItems).css("display","block");
        ...
        onHandlePressed: function (event) {
            ...
            if(itemIndex == 0)
            {
                var sliderItems=this.$(".slider-items")[0];
        ...
        onSliderSelected: function (event) {
            event.preventDefault();
            $(this.$(".slider-items")).hide();
            this.$el.find(".slider-info").css("display","block");
             if (!this.model.get("_isEnabled") || this.model.get("_isSubmitted")) return;
            ...
            for(var i=0;i<length;i++)
            {
                if(i==left){
                    ...
                    this.playAudio(this.$el.find(".slider-info"));
                }
            }
            //console.log("itemIndex: "+left);
        ...
        playAudio: function($currentElement) {
            ...
            if ($currentElement.hasClass('.slider-items')) {

                audioElement = $currentElement.find('.slider-items-audio audio')[0];
                console.log(audioElement);
            } else {
                audioElement = $currentElement.siblings('.slider-items-audio').find('audio')[0];
